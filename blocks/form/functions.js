...
function mapFormFieldsToReview() {
  ...
  // Get value (handles radio + normal inputs)
  ...
  // Set value ONLY inside review panel (prevents conflicts)
  const setFieldValue = (fieldName, value) => {
    const field = document.querySelector(`#panelcontainer-05d7daa1ba [name="${fieldName}"]`);
    if (field) {
      field.value = value;
    }
  };

  // Get radio label text (for bank names)
  ...
  // Get DOB (handles display-value properly)
  const getDOBValue = () => {
    const dobInput = document.querySelector('input[name="date_of_birth"]');
    if (!dobInput) return '';

    return (
      dobInput.getAttribute('display-value') ||
      dobInput.value ||
      ''
    );
  };
  ...
  setFieldValue(
    'loan_amount',
    loanAmount ? formatIndianCurrency(parseFloat(loanAmount)) : ''
  );
  ...
  setFieldValue(
    'tenure',
    loanTenure ? `${Math.round(parseFloat(loanTenure))} months` : ''
  );
  ...
  const firstName =
    getFieldValue('first_name_pan') || getFieldValue('first_name');

  const middleName =
    getFieldValue('middle_name_pan') || getFieldValue('middle_name');

  const lastName =
    getFieldValue('last_name_pan') || getFieldValue('last_name');
  ...
  // These exist in review panel but not in source
